...
public class TicketDubboServiceImpl implements TicketDubboService {
    ...
    @Transactional
    @Override
    public RpcResult<CreateSignTicketResponse> createSignTicket(CreateSignTicketRequest request) {
        ...
        ticket.setType(TicketType.SIGN);
    ...
    public RpcResult<CreateSyncTicketResponse> createSyncTicket(CreateSyncTicketRequest request) {
        ...
        Assert.isTrue(NumUtils.xEqualsY(accountList.size(), request.getAddressList().size()),
                String.format("工单中账户个数为 %d,签名系统账户个数为 %d,两边不一致", request.getAddressList().size(), accountList.size()));
        String airdropName = String.format("%s【%s】", request.getAirdropName(), request.getTicketNo());
        airdropService.createAirdrop(airdropName, request.getAddressList());
        Airdrop airdrop = airdropService.findByName(airdropName);
        ...
        ticket.setApplyTime(new Date());
        ticket.setStatus(TicketStatus.PENDING);
        ticket.setType(TicketType.ACCOUNT_SYNC);
        ticketService.insert(ticket);

        TicketToken ticketToken = ticketTokenService.generateTicketToken(ticket.getId(), 3600 * 24 * 3);

        return RpcResult.success(CreateSyncTicketResponse.create(request.getTicketNo(), ticketToken.getToken()));
    }
    ...
    private void buildAndSaveContractInstanceSnapshot(CreateSignTicketRequest request) {
